...
def main():
    # number = -1
    # mode = 0
    # newThread = threading.Thread(target=logging_csv_thread, args=(number, mode))
    ...
    hands = mp_hands.Hands(
        ...
        max_num_hands=1,  # Numbers of hands
    ...
    while cap.isOpened():
        ...
        results = hands.process(image)  # Getting hand landmarks from camera frames
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                # This will print out hand landmarks coordinates as an object with (x, y, z)
                ...
                # This will print out a list of [x, y] coordinates for every single hand landmarks on screen, on each frame.
                # Essentially, this will be [[x1, y1], [x2, y2],...] (up to 21 arrays since there are 21 landmarks)
                # These are pixel coordinates; the 21 landmarks can be seen on Mediapipe's docs
                # print(landmark_list)

                # Since these coordinates can have vastly different values, we need to normalize them
                ...
                # print("Current Gesture: ", keypoint_classifier_labels[hand_sign_id])
...
def label_print(string):
    print("Current gesture: ", string)
    label = string
    return label


# Socket test
def send_data(sock, UDP_IP, UDP_PORT, label, debug_var=True):
    sock.sendto(label.encode("UTF-8"), (UDP_IP, UDP_PORT))
    if (debug_var):
        print("Label: ", label, " sent!", "_" * 10)
...
# Mostly used for constant data gathering (since the only conditions for logging_csv mode "k" to work is to
# have the user in mode "k" and the number variable being )
def select_mode_(key, mode, number):
    if 48 <= key <= 57:  # 0 ~ 9
        number = key - 48
    if key == 110:  # n
        mode = 0
...
def pre_process_landmark(landmark_list):
    ...
    # After this, we'll have the relative distance between the Wrist point (Landmark index 0) and the other 20 landmarks

    # Convert to a one-dimensional list
    temp_landmark_list = list(
        itertools.chain.from_iterable(temp_landmark_list))
    # We needed to convert this into a 1D List (or a vector) since the input for the Feed-fwd neural network is a vector
...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    #                cv.LINE_AA)
    # DISABLING Finger Gesture draw temporarily

    return image


def draw_info_text_(image, brect, handedness, hand_sign_text, ):
    cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[1] - 22),
                 (0, 0, 0), -1)
